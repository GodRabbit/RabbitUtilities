	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player,
		...
		if(world.getBlockState(pos).getBlock() == RabbitBlocks.ritual_table_basic)
		{
			...
			if(world.canSeeSky(pos))
			{
				...
				if(!world.isRemote)
				{
					EntityBlaze bob=new EntityBlaze(world);
				...
				if (world.isRemote)
				{
					for (int i = 0; i < 30; ++i)
					{
						world.spawnParticle(EnumParticleTypes.FLAME, x+ (rand.nextDouble() - 0.5D) , y + rand.nextDouble() * (double)2.5 - 0.25D, z + (rand.nextDouble() - 0.5D) , (rand.nextDouble() - 0.5D) * 2.0D, -rand.nextDouble(), (rand.nextDouble() - 0.5D) * 2.0D, new int[0]);
					}
